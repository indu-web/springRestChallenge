package com.restTry.restService;
...
public class SearchServiceUnitTests {
    ...
    // 1 Item file
    @Test
    public void testSearchSvc1Item() throws Exception {
        final String itemFileName = "classpath:1Item.json";
        final File itemFile = applicationContext.getResource(itemFileName).getFile();
        searchSvc.init(itemFile);
        List<Item> itms = searchSvc.getItems();
        boolean add_result = searchSvc.addItem(itms.get(0));
        List<Item> itemsAfterAdd = searchSvc.getItems();
        float newId = itms.get(0).getId();

        assertEquals(String.format("Add new Item is %b", add_result), add_result,
                true);
        assertEquals(String.format("addItem list size: %d", itemsAfterAdd.size()), itemsAfterAdd.size(),
                itms.size()+1);

        assertEquals(String.format("New Item Id: %f", newId), 444.0, newId,1);
